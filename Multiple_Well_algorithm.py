...
def find_connections(data_grid, data_village):
    ''' Apply algorithm from paper to find which wells to open/dig and which villages to connect to it
    :param data_grid: data frame with potential well locations with corresponding suitablity and whether they have been
                        connected to a village yet
        ...
        for well_i in data_grid.index:
            ...
            for village_j in data_village.index:
                if data_village.at[village_j, 'connected'] == '' : # j unconnected
                    offer_to_facility_i.append((max(data_village.at[village_j,'budget'] - connection_cost(village_j,well_i) ,0), village_j))

                    if data_grid.at[well_i, 'connected']:  # well i is already open and j unconnected
                        if data_village.at[village_j, 'budget'] >= connection_cost(village_j, well_i): # budget of village j is bigger than connection cost to well i
                            data_village.at[village_j,'connected'] = well_i # connect village j to well i

                else: # j connected
                    well_i_prime = data_village.at[village_j, 'connected']
                    offer_to_facility_i.append((max(connection_cost(village_j, well_i_prime) - connection_cost(village_j, well_i), 0), village_j))

            if 1/data_grid.at[well_i,'suitability1'] <= sum(map(lambda x: x[0], offer_to_facility_i)): # total offer received by well i is bigger than opening costs (1/suitability)
